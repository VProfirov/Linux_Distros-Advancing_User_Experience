...
# MODIFIED
# ps-twtty-7.*.sh - Nice bash prompt playing well with other shells (zsh; fish; ...)
#   by Vasil Z. Profirov <https://github.com/VProfirov>, 2021-06-02 ...
...
#   killing the log.
# - switching from zsh to bash and zsh again might promt an error message(depending on the distro):
#	twtty:45: command not found: shopt
#   REASON: the order in which the files needed for the new shell(different than bash) initialization are loaded:
#   WORKAROUND: the ps-twtty-7.sh can be referenced in the .bashrc from the /etc/$OUR_CUSTOME_DIR ==> LOSS: other user's will not benefit from this script, root included (if not setup localy in their .bashrc)


# INSTRUCTIONS:
#	1. Create a dir in /etc/$OUR_CUSTOM_DIR place ps-twtty-7.*.sh/this file inside and create a simlink to /etc/profile.d/ using the full path to the file
#	2. Reference it in each user's .bashrc that wants this, like:
# 		#Source global definitions
#		if [ -f /etc/bashrc ]; then
#			. /etc/bashrc
#			. /etc/$OUR_CUSTOM_DIR/ps-twtty-7.*.sh
#		fi
# NOTE: the * should be replaced by the specific version and $OUR_CUSTOME_DIR can be named as you wish

if [[ "$PS1" && "$SHELL" == "/bin/bash" ]] ; then # interactive shell detection working for BASH only! 
