...
#   killing the log.
# - switching from zsh to bash and zsh again promts and error message:
...
#   WORKAROUND: the ps-twtty-7.sh can be referenced in the .bashrc from the .local/ where the file can be instead of the /etc/profile.d ==> LOSS: other user's will not benefit from this script, root included (if not setup localy)
